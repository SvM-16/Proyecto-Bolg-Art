...
export const login = async ( req, res ) =>
{
    ...
    try
    {
        ...
        if ( !isMatch )
            return res.status( 400 ).json( { message: "Error al encontrar las credenciales" } )
...
export const editProfile = async (req, res) => {
    ...
    if(!user){
        return res.status(400).json({message:'El usuario no esta encontrado'});
    }

    res.status(200).json({message:'Perfil actualicido con éxito', user})
    } catch (error) {
